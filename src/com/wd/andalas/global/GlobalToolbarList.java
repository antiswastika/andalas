...
public class GlobalToolbarList extends ToolBar {
	...
	public ToolBar createUpToolBar(final SelectHandler insertHandler, final SelectHandler deleteHandler, final SelectHandler refreshHandler, final SelectHandler printHandler, final SelectHandler exportHandler, final ChangeHandler clearSearchHandler, final SelectHandler searchHandler, final SelectHandler windowHandler) {
		...
		if (insertHandler != null) {
			TextButton btn11 = new TextButton(localText.labelButtonMap().get("label.button.insert"));
			btn11.setIcon(imageResource.btnAdd());
			btn11.setToolTip(localText.labelButtonMap().get("label.button.insert.tooltip"));
			btn11.addSelectHandler(insertHandler);
			toolbarNya.add(btn11);
		}
		if (deleteHandler != null) {
			TextButton btn12 = new TextButton(localText.labelButtonMap().get("label.button.delete"));
			btn12.setIcon(imageResource.btnDelete());
			btn12.setToolTip(localText.labelButtonMap().get("label.button.delete.tooltip"));
			btn12.setEnabled(false);
			btn12.addSelectHandler(deleteHandler);
			toolbarNya.add(btn12);
		}
		if (refreshHandler != null) {
			TextButton btn13 = new TextButton(localText.labelButtonMap().get("label.button.refresh"));
			btn13.setIcon(imageResource.btnRefresh());
			btn13.setToolTip(localText.labelButtonMap().get("label.button.refresh.tooltip"));
			btn13.addSelectHandler(refreshHandler);
			toolbarNya.add(btn13);
		}
		if (printHandler != null) {
			TextButton btn14 = new TextButton(localText.labelButtonMap().get("label.button.print"));
			btn14.setIcon(imageResource.btnPrinter());
			btn14.setToolTip(localText.labelButtonMap().get("label.button.print.tooltip"));
			btn14.addSelectHandler(printHandler);
			toolbarNya.add(btn14);
		}
		if (exportHandler != null) {
			TextButton btn15 = new TextButton(localText.labelButtonMap().get("label.button.export"));
			btn15.setIcon(imageResource.btnExport());
			btn15.setToolTip(localText.labelButtonMap().get("label.button.export.tooltip"));
		...
		if (clearSearchHandler != null) {
			...
			cbk1.setWidth(15);
			cbk1.setToolTip(localText.labelButtonMap().get("label.button.clearSearch.tooltip"));
			cbk1.addChangeHandler(clearSearchHandler);
			toolbarNya.add(cbk1);
		}
		if (searchHandler != null) {
			TextButton btn21 = new TextButton(localText.labelButtonMap().get("label.button.search"));
			btn21.setIcon(imageResource.btnSearch());
			btn21.setToolTip(localText.labelButtonMap().get("label.button.search.tooltip"));
			btn21.addSelectHandler(searchHandler);
			toolbarNya.add(btn21);	
		}
		if (windowHandler != null) {
			TextButton btn22 = new TextButton(localText.labelButtonMap().get("label.button.window"));
			btn22.setIcon(imageResource.btnWindow());
			btn22.setToolTip(localText.labelButtonMap().get("label.button.window.tooltip"));
	...
	public ToolBar createDownToolBar(final SelectHandler saveHandler, final SelectHandler resetHandler, final SelectHandler closeHandler, final SelectHandler infoHandler, final List<SelectHandler> customHandlerList, final List<String> customHandlerTextList,  final List<ImageResource> customHandlerIconResourceList) {
		...
		if (infoHandler != null) {
			...
			btn30.setToolTip(localText.labelButtonMap().get("label.button.info.tooltip"));
			btn30.addSelectHandler(infoHandler);
			toolbarNya.add(btn30);
		}
		if (resetHandler != null) {
			TextButton btn31 = new TextButton(localText.labelButtonMap().get("label.button.reset"));
			btn31.setIcon(imageResource.btnRefresh());
			btn31.setToolTip(localText.labelButtonMap().get("label.button.reset.tooltip"));
		...
		if (saveHandler != null) {
			TextButton btn38 = new TextButton(localText.labelButtonMap().get("label.button.saveUpdateSubmit"));
			btn38.setIcon(imageResource.btnDiscBlue());
			btn38.setToolTip(localText.labelButtonMap().get("label.button.saveUpdateSubmit.tooltip"));
			btn38.addSelectHandler(saveHandler);
			toolbarNya.add(btn38);
		}
		if (closeHandler != null) {
			TextButton btn39 = new TextButton(localText.labelButtonMap().get("label.button.close"));
			btn39.setIcon(imageResource.btnExit());
			btn39.setToolTip(localText.labelButtonMap().get("label.button.close.tooltip"));
